...
def get_json_completion_from_messages(client, messages, model, temperature=0.7):
    client = client
...
def genarating_outline(client, keywords, learning_intention, num_lessons, language, model):
    ...
    user_message = f"""You are a great AI teacher and linguist,
            skilled at generating course outline based on keyword of the course.
            Based on keyword provided, you should carefully design a course outline. 
...
def decorate_suggested_questions_supervisor(language, question_list):
    if language == 'Hindi':
        ...
        हम आपके शिक्षा की देखभाल करते हैं। निम्नलिखित आपके शिक्षा स्थिति के अनुसार सिफारिशित प्रश्न हैं::
...
def decorate_suggested_questions_assistant(lesson_count, language, question_list):
    if language == 'Hindi':
        decorated_suggest_question = f"""
         {lesson_count}वीं पाठ के आधार पर, निम्नलिखित प्रश्नों की आपके लिए सिफारिश की गई हैं:
