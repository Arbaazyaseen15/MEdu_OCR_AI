...
def initialize_outline(client, temp_file_paths, learning_intention, num_lessons, language, model):
    with st.spinner('Generating Course Outline...'):
        summarized_materials = get_keywords(temp_file_paths)[0]
...
def display_main_page(is_visualized):
    description_1 = """
            :grey[=>:] Final Year Project By Hritik Singh, Arbaaz Yaseen, Shaikh Mohammad.

            ### ✨ Key features                                           
                                                    
            - 🧑‍🏫 **Language Accessibility**: <font color = 'grey'>Generated from any learning materials in your own preferred language (**.md or .pdf**)!</font>
            - 📚 **Content Translation**: <font color = 'grey'>Whether it's math, physics, literature, history or coding, it covers it all.</font>
            - ⚡️ **Cultural Integration**: <font color = 'grey'>Based on the materials provided.AI-driven multilingual education promotes cultural integration by facilitating the exchange of ideas, perspectives, and knowledge across linguistic boundaries</font>
            - 🔮 **Customized Learning Experience**: <font color = 'grey'>Multilingual education with AI and OCR allows for personalized learning experiences tailored to individual language proficiency levels.</font>
            """
    description_2 = """
    ### 🏃‍♂️To Get started!

    1. **Config basic info**: <font color = 'grey'>Give the Application your own OpenAI API key and upload your learning materials.</font>
    """

    description_3 = """
    2. **Customize your learning journey**: <font color = 'grey'>Provide your preferred teaching style, topic, lesson count and language. All of these are optional.</font>
    """

    description_4 = """
    3. **Start course generating**: <font color = 'grey'>Click "Start Learning" button below.</font>
    """
    ...
    if is_visualized:
        with main_page.container():
            ...
            with st.expander('⚙️ Customize my course'):
                ...
                learner_input = st.text_input('(Optional) Please enter what you want to learn, you can enter keywords or phrases, and this may help the application create better lessons for you :)', 
                                              placeholder = 'Use comma "," to split the different keywords/phrases.')
                ss.language = 'English'
                Hindi = st.checkbox('Output in Hindi')
            
            st.markdown(description_4, unsafe_allow_html=True)
            btn_start = st.button('🔮 Start Learning')
        return api_key, use_35, added_files, num_lessons, custom_options, learner_input, Hindi, btn_start
    else:
        main_page.empty()
        return None
...
def display_current_status_col2():
    st.caption(''':blue[Multilingual AI Education]: Ask any questions related to pdf and get direct answers. :''')
...
def display_warning_not_started():
    ...
    warning_not_started.warning("The learning haven't started yet, please start to learn first!")
    time.sleep(2)
    warning_not_started.empty()

def display_warning_started():
    general_warning = st.empty()
    general_warning.markdown("The learning has already started, please touch 'Next lesson' button!")
    time.sleep(2)
    general_warning.empty()

def display_warning_api_key():
    warning_api_key = st.empty()
    warning_api_key.markdown("Please enter the OpenAI API Key first.")
    time.sleep(2)
